...
# Functional Profiling of Candidate ECM-DEGs: GO (BP, CC, MF) and KEGG Pathways
# ==============================================================================
...
cat("=================================================================\n")
cat("STEP 6: FUNCTIONAL ENRICHMENT OF ECM-DEGs (CLUSTERPROFILER)\n")
cat("=================================================================\n")

if (!file.exists("data/GSE125498_ECM_DEGs.RData")) {
  stop("ECM-DEG results not found! Please run 03_matrisome_filtering.R first.")
}
load("data/GSE125498_ECM_DEGs.RData")
...
go_bp <- run_go_enrichment(target_genes, "BP", "ECM-DEGs")
go_cc <- run_go_enrichment(target_genes, "CC", "ECM-DEGs")
go_mf <- run_go_enrichment(target_genes, "MF", "ECM-DEGs")
...
if (!is.null(go_bp) && nrow(as.data.frame(go_bp)) > 0) {
  ...
  p_bp <- dotplot(go_bp, showCategory = min(15, nrow(df_bp)), title = "GO Biological Process (ECM-DEGs)") +
...
if (!is.null(go_cc) && nrow(as.data.frame(go_cc)) > 0) {
  ...
  p_cc <- dotplot(go_cc, showCategory = min(15, nrow(df_cc)), title = "GO Cellular Component (ECM-DEGs)") +
    theme_bw(base_size = 12) +
    scale_color_gradient(low = "#d73027", high = "#4575b4")
...
if (!is.null(go_mf) && nrow(as.data.frame(go_mf)) > 0) {
  ...
  p_mf <- dotplot(go_mf, showCategory = min(15, nrow(df_mf)), title = "GO Molecular Function (ECM-DEGs)") +
    theme_bw(base_size = 12) +
    scale_color_gradient(low = "#d73027", high = "#4575b4")
...
if (!is.null(entrez_mapping) && nrow(entrez_mapping) > 0) {
  ...
  if (!is.null(kegg_res) && nrow(as.data.frame(kegg_res)) > 0) {
    ...
    p_kegg <- dotplot(kegg_res, showCategory = min(15, nrow(df_kegg)), title = "KEGG Pathway Enrichment (ECM-DEGs)") +
      theme_bw(base_size = 12) +
      scale_color_gradient(low = "#d73027", high = "#4575b4")
...
go_all_bp <- enrichGO(gene          = all_deg_genes,
                      OrgDb         = org.Hs.eg.db,
                      keyType       = "SYMBOL",
                      ont           = "BP",
                      pAdjustMethod = "BH",
...
if (!is.null(go_all_bp) && nrow(as.data.frame(go_all_bp)) > 0) {
  ...
  p_all_bp <- dotplot(go_all_bp, showCategory = 15, title = "GO Biological Process (All DEGs: GSE125498)") +
    theme_bw(base_size = 12) +
    scale_color_gradient(low = "#d73027", high = "#4575b4")
  ggsave("results/figures/Enrichment_05_Global_DEGs_GO_BP.png", p_all_bp, width = 9, height = 7, dpi = 300)
}

save(go_bp, go_cc, go_mf, file = "data/GSE125498_Enrichment_results.RData")
